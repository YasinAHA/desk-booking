...
const reservationErrorMessages: Record<string, string> = {
  ...
  DATE_IN_PAST: "No se puede operar con reservas en una fecha pasada.",
...
const cancelReservationErrorMessages: Record<string, string> = {
  ...reservationErrorMessages,
  DATE_IN_PAST: "No se puede cancelar una reserva en una fecha pasada."
};
...
export function mapCreateReservationErrorToMessage(
  error: unknown,
  fallbackMessage: string
): string {
  if (!(error instanceof ApiError)) {
  ...
  return createReservationErrorMessages[error.code] ?? error.message ?? fallbackMessage;
}

export function mapCancelReservationErrorToMessage(
  error: unknown,
  fallbackMessage: string
): string {
  if (!(error instanceof ApiError)) {
  ...
  return cancelReservationErrorMessages[error.code] ?? error.message ?? fallbackMessage;
}
