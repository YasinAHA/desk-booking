...
test("POST /auth/register returns 403 for domain not allowed", async () => {
    ...
    const res = await app.inject({
        method: "POST",
        url: "/auth/register",
        payload: { email: "user@other.com", password: "123456" },
    });
...
test("POST /auth/register returns 409 when already confirmed", async () => {
    const app = await buildTestApp(async () => {
...
test("POST /auth/login returns 403 when not confirmed", async () => {
